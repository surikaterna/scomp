...
function createHandlers(): ScompControlPlaneRouteHandlers {
  return {
    discover: async (request) => ({
      services: request.includeRoutes ? [{ name: "users", routes: ["users.getUser"] }] : [{ name: "users" }],
      node: { id: "node-a" },
    }),
    ...
    health: async (request) => ({
      status: "ok",
      checks: request.verbose ? [{ name: "router", status: "ok" }] : undefined,
      node: { id: "node-a" },
    }),
  };
}
...
describe("control-plane router composition", () => {
  ...
  it("filters deep health checks by target service when requested", async () => {
    ...
    const health = createNodeLocalHealthHandler(appRouter, {
      nodeId: "node-health",
      checks: [async () => ({ name: "users.db", status: "ok" }), async () => ({ name: "orders.db", status: "down" })],
    });

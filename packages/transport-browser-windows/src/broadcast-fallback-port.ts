...
export class InMemoryBrokerPort implements BrokerPortLike {
  private readonly listeners = new Set<(event: { data: BrowserWindowsProtocolMessage }) => void>();

  constructor(private readonly sender: (message: BrowserWindowsProtocolMessage) => void) {}

  postMessage(message: BrowserWindowsProtocolMessage): void {
    this.sender(message);
  }

  addEventListener(_type: "message", listener: (event: { data: BrowserWindowsProtocolMessage }) => void): void {
    this.listeners.add(listener);
  }

  removeEventListener(_type: "message", listener: (event: { data: BrowserWindowsProtocolMessage }) => void): void {
